...
const NetSwap: FC<any> = ({ }) => {
    ...
    const getTokenMetadata = async (addressToken: string) => {
        ...
        try {
            if (addressToken == "So11111111111111111111111111111111111111112") throw "sol"
    ...
    async function updateInfoTokens(infoTokens: any[]) {
        ...
        try {
            ...
            const updatedTokens = await Promise.all(infoTokens.map(async (token, index) => {
    ...
    const selectToken = (addressToken: string) => {
        ...
        if (tokensDuplicate.includes(addressToken)) {
            const tokensRemaining = tokensDuplicate.filter(token => token != addressToken)
        ...
        <div style={{ minWidth: 240 }} className="mb-8 w-[600px]  flex  flex-col gap-5">
            <div className="w-full border-b border-gray-500 pb-4 ">
                <div className="flex gap-5 flex-wrap mb-3" >
                    {infoTokens.length > 0 && infoTokens.map((token, index) => {
                        if (token && token.tokenBalance > 1 && token && token?.symbol) {

                            return <div key={index} onClick={() => selectToken(token.mintAddress)} className=" relative flex gap-3 items-center rounded-lg bg-purple-600 p-2 cursor-pointer hover:scale-110 transition-all">
                                {tokensSelected.includes(token.mintAddress) &&

                                    <div className="p-1 absolute bg-yellow-400 top-[-16px] right-0 text-black">
                                        {tokensSelected[1] && tokensSelected[1] == token.mintAddress ? "TokenB" : "Token A"}
                                    </div>
                                }
                                <img src={token?.image ? token?.image : token?.logoURI ? token?.logoURI : "question.webp"} className="w-[30px] h-[30px] rounded-full" alt="" />
                                <div className="flex flex-col gap-1 ">
                                    <span className="font-bold">{token?.symbol ? token.symbol : "Unrecognized Token"}</span>
                ...
                <div className="flex gap-3">
                    <div className="flex flex-col gap-2 flex-1 w-[50%]">
                        ...
                        <input name="amountA" onChange={(e) => {
                            const value = e.target.value;
                            const nameKey = "amountA"
                            setValueToPoolInfo(nameKey, value)
                        }
                        } placeholder="Amount Token A" className="mb-4 "></input>
                    </div>
                    <div className="flex flex-col gap-2 flex-1 w-[50%]">
                        <span>Amount Token B</span>
                        <input name="amountB" type="number" onChange={(e) => {
                            const value = e.target.value;
                            const nameKey = "amountB"
                            setValueToPoolInfo(nameKey, value)
                        }
                        } placeholder="Amount Token B" className="mb-4 "></input>
                    </div>
                ...
                <div className="flex gap-3">
                    <div className="flex flex-col gap-2 flex-1">
                        ...
                        <input name="curveType" onChange={(e) => {
                            const value = e.target.value;
                            const nameKey = "curveType"
                            setValueToPoolInfo(nameKey, value)
                        }
                        } placeholder="Curve Type" className="mb-4"></input>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 flex-1">
                        ...
                        <select
                            ...
                            {networks.map(network => {
                                return <option selected={poolInfo.networkType == network} value={network}>{network}</option>
                            })}
                        </select>
                ...
                <div className="flex gap-3">
                    <div className="flex flex-col gap-2 flex-1">
                        ...
                        <input name="ownerFeeAddress" onChange={(e) => {
                            const value = e.target.value;
                            const nameKey = "ownerFeeAddress"
                            setValueToPoolInfo(nameKey, value)
                        }
                        } placeholder="Owner Fee Address" className="mb-4 flex-1"></input>

                    </div>

                    <div className="flex flex-col gap-2 flex-1">
                        <span>Fee</span>
                        <input name="fee"  readOnly value={fee ? `${fee} SOL` : "0 SOL"}
                            placeholder="Fee" className="mb-4 flex-1 text-yellow-500"></input>
                    </div>
